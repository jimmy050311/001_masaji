<template lang="">
    ...
    <ol class="breadcrumb float-xl-end">
    	<li class="breadcrumb-item"><a href="javascript:;">編輯活動</a></li>
    	<li class="breadcrumb-item active">促銷管理</li>
    </ol>
    <!-- END breadcrumb -->
    <!-- BEGIN page-header -->
    <h1 class="page-header">編輯活動<small></small></h1>
    <!-- END page-header -->
    <!-- BEGIN panel -->
    <panel>
        <panel-body>
            <p class="text-invers" style="margin-bottom: 5px"><i class="fas fa-bullhorn fa-fw"></i>&nbsp;請點擊下方按鈕編輯活動</p>
            <button type="submit" class="col-md-6 btn btn-primary w-100px" @click="submit()">編輯活動</button>
        </panel-body>
    </panel>
    <panel>
        <panel-body>
            ...
            <div class="col-md-12">
                <input class="form-control" placeholder="請輸入活動名稱" v-model="eventData.code"/>
            </div>
...
<script setup>
    ...
    onMounted(async() => {
        ...
        eventStore.fetchEventDetail(eventId.value).then((response) => {
            eventData.name = response.name
            eventData.category_id = response.category_id
            eventData.discount = parseFloat(response.discount)
            eventData.desc = response.desc
            eventData.status = response.status
            eventData.start_date = response.start_date
            eventData.end_date = response.end_date
            eventData.code = response.code
        })
    })
    ...
    function submit() {
        ...
        eventStore.fetchEditEvent(eventId.value, data).then((response) => {
            if(response) {
