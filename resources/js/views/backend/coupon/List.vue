<template lang="">
    <div>
		...
		<!-- END breadcrumb -->
		<!-- BEGIN page-header -->
		<h1 class="page-header">優惠券列表<small></small></h1>
		<!-- END page-header -->
        <!-- BEGIN panel -->
        <panel>
          <panel-body>
            <p class="text-invers mb-2" style="text-align-center"><i class="fas fa-bullhorn fa-fw"></i>&nbsp;請輸入優惠券名稱篩選資料</p>
            <n-input
                class=""
                round
                clearable
...
<script setup>
    ...
    const tableData = computed(() => {
      return couponStore.couponData.data
    })
    const page = computed(() => {
      return couponStore.couponData.dataPage
    })
    ...
    onMounted(() => {
      ...
      couponStore.fetchCoupon(currentPage.value, data)
    })
    ...
    function edit(id) {
        router.push({ name: 'coupon-edit', params: { id: id } })
    }
