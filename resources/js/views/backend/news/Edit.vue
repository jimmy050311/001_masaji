<template lang="">
    ...
    <ol class="breadcrumb float-xl-end">
    	<li class="breadcrumb-item"><a href="javascript:;">編輯最新消息</a></li>
    	<li class="breadcrumb-item active">前台管理</li>
    </ol>
    <!-- END breadcrumb -->
    <!-- BEGIN page-header -->
    <h1 class="page-header">編輯最新消息 <small></small></h1>
    <!-- END page-header -->
    <!-- BEGIN panel -->
    <panel>
        <panel-body>
            <p class="text-invers" style="margin-bottom: 5px"><i class="fas fa-bullhorn fa-fw"></i>&nbsp;請點擊下方按鈕編輯最新消息</p>
            <button type="submit" class="col-md-6 btn btn-primary w-100px" @click="submit()">編輯最新消息</button>
        </panel-body>
    </panel>
    <panel>
        <panel-body>
            <div class="row">
                ...
                <label class="form-label col-form-label col-md-12 d-flex fs-7">輪播圖片</label>
                <div class="col-md-12">
...
<script setup>
    ...
    onMounted(async() => {
        await newsStore.fetchNewsDetail(newsId.value).then((response) => {
            newsData.title = response.title
            newsData.status = response.status
            newsData.content = response.content
            image.push({ image: response.image })
            //初始化ckeditor
            ckeditorRef.value.showContent(newsData.content)
        })
    ...
    function submit() {
        ...
        newsStore.fetchEditNews(newsId.value, data).then((response) => {
            if(response) {
